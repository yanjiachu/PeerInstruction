...
Page({
  ...
  onLoad(options) {
    ...
    this.fetchCourseDetails(courseID)
      .then(result => {
        console.log(result);
        const ownerID = result.teacherID;
        if(ownerID === getApp().globalData.userID) {
          this.setData({
            isMyCourse: true
          })
        } 
  ...
  onEditTap : function (params) {
    const courseID = this.data.courseID;
    if(!this.data.isMyCourse) {
      wx.showToast({
        title: '非本人课程',
        icon: 'error',
      })
      ...
      wx.navigateTo({
        url: `/pages/teacher/course/courseEdit/courseEdit?courseID=${courseID}`,
      })
    }
  },
  ...
  onMemberTap : function (params) {
    ...
    if(!this.data.isMyCourse) {
      ...
      wx.navigateTo({
        url: `/pages/teacher/course/courseMember/courseMember?courseID=${courseID}&courseName=${encodeURIComponent(courseName)}`,
      })
    }
  },
  ...
  onClassTap : function(e) {
    ...
    wx.navigateTo({
      url: `/pages/teacher/class/class?classId=${classId}&courseId=${courseId}`
    })};
  },

  onCreateClassTap : function() {
    const courseId = this.data.courseID;
    const courseName = this.data.courseName;
    wx.navigateTo({
      url: `/pages/teacher/addClass/addClass?courseId=${courseId}&courseName=${encodeURIComponent(courseName)}`,
    })
  },
  ...
  onFileTap : function () {
    ...
    wx.navigateTo({
      url: `/pages/teacher/course/courseFile/courseFile?courseID=${courseID}&courseName=${encodeURIComponent(courseName)}`,
    })}    
  },
